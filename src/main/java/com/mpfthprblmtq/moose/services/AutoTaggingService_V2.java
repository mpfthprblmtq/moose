...
public class AutoTaggingService_V2 {
    ...
    public void autoTag(List<Song> songs) {
        for (Song song : songs) {
            ...
            int row = Moose.getSongController_v2().getRow(index);

            // title
            Moose.getSongController_v2().setTitle(index, title);
            Moose.getFrame().getTable().setValueAt(title, row, TABLE_COLUMN_TITLE);

            // artist
            Moose.getSongController_v2().setArtist(index, artist);
            Moose.getFrame().getTable().setValueAt(artist, row, TABLE_COLUMN_ARTIST);

            // album
            Moose.getSongController_v2().setAlbum(index, album);
            Moose.getFrame().getTable().setValueAt(album, row, TABLE_COLUMN_ALBUM);

            // album artist
            Moose.getSongController_v2().setAlbumArtist(index, albumArtist);
            Moose.getFrame().getTable().setValueAt(albumArtist, row, TABLE_COLUMN_ALBUM_ARTIST);

            // year
            Moose.getSongController_v2().setYear(index, year);
            Moose.getFrame().getTable().setValueAt(year, row, TABLE_COLUMN_YEAR);

            // genre
            Moose.getSongController_v2().setGenre(index, genre);
     ...
     * @return the title
     */
    protected String getTitleFromFile(File file) {
     ...
     * @return the artist
     */
    protected String getArtistFromFile(File file) {
        // check if we already have an artist from the filename processing
        Song song = Moose.getSongController_v2().getSongs().get(Moose.getSongController_v2().getIndex(file));
     ...
     * @return the album
     */
    protected String getAlbumFromFile(File file) {
     ...
     */
    protected String getAlbumArtistFromFile(File file) {
     ...
     * @return the year
     */
    protected String getYearFromFile(File file) {
     ...
     */
    protected String getTracksFromFile(File file) {
     ...
     */
    protected String getDisksFromFile(File file) {
     ...
     * @return the disks string in X/XX format
     */
    protected String getGenreFromFile(Song song) {
    ...
    private String getCommonGenreForArtist(File file) {
        ...
        for (File fileInList : files) {
            ...
            if (fileInList.getName().endsWith(".mp3")) {
                ...
                Song song = Moose.getSongController_v2().getSongService().getSongFromFile(fileInList);
                if (StringUtils.isNotEmpty(song.getGenre())) {
...
//                    genreCounts.getOrDefault(song.getGenre(),
//                            genreCounts.getOrDefault(song.getGenre(), 1) + 1);
        ...
        if (CollectionUtils.isNotEmpty(songsToReprocess)) {
            ...
            for (Song song : songsToReprocess) {
                if (!ImageSearchQuery.contains(queries, song.getAlbum())) {
                    ...
                    query.getRows().add(Moose.getSongController_v2().getRow(song.getIndex()));
                    queries.add(query);
                } else {
                    ...
                    queries.get(ImageSearchQuery.getIndex(queries, song.getAlbum()))
                            .getRows().add(Moose.getSongController_v2().getRow(song.getIndex()));
                }
            }
                    ...
                    if (StringUtils.isNotEmpty(url)) {
                        ...
                        if (outputFile != null && outputFile.exists()) {
                            for (Integer row : query.getRows()) {
                                Song song = Moose.getSongController_v2().getSongs().get(
                                        Moose.getSongController_v2().getIndex(row));
    ...
    public void addCoverForFile(Song song, File cover) {
        ...
        Moose.getSongController_v2().setAlbumImage(song.getIndex(), bytes);

        // update graphics
        Moose.getFrame().getTable().setValueAt(
                ImageUtils.getScaledImage(bytes, 100),
                Moose.getSongController_v2().getRow(song.getIndex()), TABLE_COLUMN_ALBUM_ART);
    }
    ...
    public void autoAddTrackAndDiskNumbers(List<Song> songs) {
        ...
        for (Song song : songs) {
            ...
            int row = Moose.getSongController_v2().getRow(song.getIndex());
            ...
            if (trackArr.length == 2) {
                Moose.getSongController_v2().setTrack(song.getIndex(), trackArr[0]);
                Moose.getSongController_v2().setTotalTracks(song.getIndex(), trackArr[1]);
                Moose.getFrame().getTable().setValueAt(tracks, row, TABLE_COLUMN_TRACK);
            } else {
                Moose.getSongController_v2().setTrack(song.getIndex(), StringUtils.EMPTY);
                Moose.getSongController_v2().setTotalTracks(song.getIndex(), StringUtils.EMPTY);
            }

            // disk
            String[] diskArr = disks.split("/");
            if (diskArr.length == 2) {
                Moose.getSongController_v2().setDisk(song.getIndex(), diskArr[0]);
                Moose.getSongController_v2().setTotalDisks(song.getIndex(), diskArr[1]);
                Moose.getFrame().getTable().setValueAt(disks, row, TABLE_COLUMN_DISK);
            } else {
                Moose.getSongController_v2().setDisk(song.getIndex(), StringUtils.EMPTY);
                Moose.getSongController_v2().setTotalDisks(song.getIndex(), StringUtils.EMPTY);
            }
        }
    }
    ...
    public void setArtistOnSong(File file, String artist) {
        int index = Moose.songController_v2.getIndex(file);
        if (index != -1) {
            Moose.getSongController_v2().setArtist(index, artist);
        }
    }
